...
create view vw_player_stats as select p.playerid, p.lastname||','||p.firstname as playername,s.year,s.stint,s.position,s.gamesplayed,s.goals,s.assists,s.penaltyminutes from players p join scoring s on s.playerid = p.playerid;
